...
module.exports = (robot) ->
   ...
   robot.hear /!slap (.*)/i, (res) ->
     ...
     now = new Date()
     if target == "leafwind"
       res.send "打了 #{target} 一巴掌。不痛不癢。"
     else
       if trollersDict[target]["health"] <= 0
         res.send "#{target} 已死，有事燒紙"
         if (now - trollersDict[target]["spawn"]) / 1e3 > 10 * 60
           trollersDict[target]["health"] = 90
           trollersDict[target]["spawn"] = now
           res.send "#{target} 重生之後馬上被賞一巴掌。HP 剩下 #{trollersDict[target]["health"]}。"
       else
         trollersDict[target]["health"] -= 10
         res.send "打了 #{target} 一巴掌。HP 剩下 #{trollersDict[target]["health"]}。"
